		...
		public:
			...
			bool write(detail::log_entry &&entry, int64_t &index)
			{
				...
				if (!current_file_.is_open())
					check_apply(current_file_.open(path_ + std::to_string(entry.index_)+".log"));
			...
			bool get_log_entry(int64_t index, log_entry &entry)
			{
				...
				if (get_entry_from_cache(entry, index))
					return true;
				if (index <= current_file_.get_log_start() &&
					current_file_.get_last_log_index() <= index)
					return current_file_.get_entry(index, entry);
				auto itr = logfiles_.lower_bound(index);
				if (itr == logfiles_.end())
					return false;
				if(itr->second.get_last_log_index() < index)
					return false;
				if (!itr->second.get_entry(index, entry))
					return false;
				return true;
			}
			std::list<log_entry> get_log_entries(int64_t index, std::size_t count = 10)
			{
				...
				do
				{
					...
					auto itr = logfiles_.upper_bound(index-1);
					if (itr == logfiles_.end())
						return log_entries;
			...
			{
				std::lock_guard<std::mutex> lock(mtx_);
				for (auto itr = logfiles_.upper_bound(index); itr != logfiles_.end(); ++itr)
				{
					if (itr->second.get_log_start() < index &&
				...
				for (auto itr = logfiles_.lower_bound(index); itr != logfiles_.end(); ++itr)
				{
					itr->second.rm();
				}
				last_index_ = index;
		...
		private:
			...
			bool check_current_file_size()
			{
				if (current_file_.size() > max_file_size_  )
				{
					...
					check_apply(current_file_.open(path_ + std::to_string(last_index_ + 1) + ".log"));
				}
				return true;
			}
